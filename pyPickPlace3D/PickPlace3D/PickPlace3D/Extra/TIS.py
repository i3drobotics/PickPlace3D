...
class TIS:
    ...
    def __init__(self,serial, width, height, framerate, color):
        ...
        source_name = "source" + serial
        sink_name = "sink" + serial

        p = 'tcambin serial="%s" name=%s ! video/x-raw,format=%s,width=%d,height=%d,framerate=%d/1' % (serial,source_name,pixelformat,width,height,framerate,)
        p += ' ! videoconvert ! appsink name=%s' % (sink_name,)
        # p += ' ! appsink name=sink'
        ...
        self.source = self.pipeline.get_by_name(source_name)
